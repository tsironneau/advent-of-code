...
public class Day_12_2 {
    ...
    public static void main(String[] args) {
        ...
        Set<WeightedPoint> startingPoints = values.stream().filter(
                v -> v.weight == 97
        ).collect(Collectors.toSet());

        startingPoints.stream().mapToInt(
                              p -> computeRouteLength(points, end, connections, nodes, p)
                      ).min()
                      .ifPresent(System.out::println);
    }

    private static int computeRouteLength(Map<IPoint, WeightedPoint> points, IPoint end, Map<WeightedPoint, Set<WeightedPoint>> connections, Map<WeightedPoint, WeightedPoint> nodes, WeightedPoint from) {
